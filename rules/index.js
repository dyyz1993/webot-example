...
var _ = require('underscore')
  ._;
var search = require('../lib/support')
  .search;
var geo2loc = require('../lib/support')
  .geo2loc;
...
module.exports = exports = function(webot) {
  ...
  webot.waitRule('wait_guess', function(info) {
    ...
    //重试
    info.rewait();
    return (r > num ? '大了' : '小了') + ',还有' + (2 - rewaitCount) +
      '次机会,再猜.';
  });
  ...
  webot.waitRule('wait_timeout', function(info) {
    if (new Date()
      .getTime() - info.session.wait_begin > 5000) {
  ...
  webot.set('timeout', {
    ...
    pattern: 'timeout',
    handler: function(info) {
      info.session.wait_begin = new Date()
        .getTime();
  ...
  webot.waitRule('list-2', function(info) {
    var r = Number(info.text);
    // console.log(info);
    // 用户不想玩了...
    if (isNaN(r)) {
    ...
    var replyWord = '您当前选择的是'

    info.session.antes.push({
      type: r,
      money: 0,
      count: 0
    })
    var type = transYXX(r);
    replyWord += (type + ",请输入您要下注的金额");
    // console.log()
    info.wait('list-2-3');
    return replyWord;
  });
  ...
  webot.waitRule('list-2-3', function(info) {
    ...
    var ante = info.session.antes[info.session.antes.length - 1];
    ante.money = r;
    info.wait('list-2-3-4');
    // info.session.antes['type'] = r;
    return '你压了 ' + r + '金币在' + transYXX(ante.type) + '上面\n' +
      "是否要继续下注\n" +
      "1-继续\n" +
      "2-开注"
  });

  webot.waitRule('list-2-3-4', {
    '1': function(info) {
      info.wait('list-2');
      return '请选择鱼虾蟹:\n' +
        '1 - 鱼\n' +
    ...
    '2': function(info) {
      ...
      var text = " 编号 | 类型 |     筹码      |       中奖金额 \n";
      for (var i = 0; i < antes.length; i++) {
        var ante = antes[i];
        text += "  " + (i + 1) + "   |  " + transYXX(ante.type) +
          "  |  " +
          ante.money +
          "   |   " + ante.money * ante.count + " \n"
      }
      info.session.antes = [];
      return '开奖结果为 ' + transYXX(randNum[0]) +
        "、" + transYXX(randNum[1]) + "、" + transYXX(randNum[1]) + "\n" +
        "开奖结果为:\n" + text
    }
